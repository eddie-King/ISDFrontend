...
const Bottoms = () => {
    ...
    useEffect(()=>{
      axios.get("http://localhost:8088/products/list?categoryId=3")
      . then(response =>{
            ...
            <div className="masonry-grid">
                {products && products.data && products.data.map((product) => (
                    <div className="masonry-grid-item" key={product.id}>
                        <Card>
                            <Card.Img variant="top" className='imagesec' src={product.imageUrl} alt={product.name} style={{ width: '100%', height: '200px', objectFit: 'contain' }} />
                            <Card.Body className='dessec'>
